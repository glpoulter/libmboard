/* $Id: addmessage.c 2108 2009-09-14 14:14:31Z lsc $ */
/*!
 * \file parallel/addmessage.c
 * \code
 ...
 * \endcode
 * 
 * \brief Parallel implementation of MB_AddMessage()
 * 
 */
...
int MB_AddMessage(MBt_Board mb, void *msg) {
    ...
    {
        P_FUNCFAIL("Cannot add to message to null board (MB_NULL_MBOARD)");
    ...
    if (!msg)
    {
        P_FUNCFAIL("NULL message pointer");
        return MB_ERR_INVALID;
    }
    ...
    {
        P_FUNCFAIL("Internal error. pl_newnode() returned code %d", rc);
